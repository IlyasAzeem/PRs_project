...
def get_classifiers():
    return {
        ...
        'LinearSVC': LinearSVC(max_iter=2000, dual=False),
...
df = df.loc[df.Project_Name == 'django']
target = 'label'
...
predictors = [x for x in df.columns if x not in [target, 'PR_accept', 'PR_Date_Created_At', 'PR_Time_Created_At',
                                                 'PR_Date_Closed_At', 'PR_Time_Closed_At', 'Project_Name']]
...
def train_bagging_classifier():
    results = pd.DataFrame(columns=['Model', 'P_respond', 'P_accept', 'P_reject', 'R_respond', 'R_accept', 'R_reject',
                                    'f1_respond', 'f1_accept', 'f1_reject', 'Avg_Pre', 'Avg_Recall', 'Avg_f1_Score',
                                    'Test_Accuracy', 'Train_Accuracy', 'Training_time', 'Testing_time'])
    ...
    for name, value in classifiers.items():
        ...
        start_training_time = t.time()
        model = BaggingClassifier(clf, n_estimators=100, random_state=42)
        if name == 'linerSVC' or name == 'LogisticRegression':
        ...
        results = results.append(
            {'Model': name,
             'P_respond': precision[1], 'P_accept': precision[0], 'P_reject': precision[2], 'R_respond': recall[1],
             'R_accept': recall[0], 'R_reject': recall[2], 'f1_respond': fscore[1], 'f1_accept': fscore[0],
             'f1_reject': fscore[2],
             'Avg_Pre': precision_avg, 'Avg_Recall': recall_avg, 'Avg_f1_Score': fscore_avg,
             'Test_Accuracy': test_accuracy,
             'Train_Accuracy': train_accuracy,
    ...
    results.to_csv('Results/results_bagging.csv', sep=',', encoding='utf-8', index=False)
...
def start_train_models():
    ...
    for name, value in classifiers.items():
            ...
            print('Classifier: ', name)
            if name == 'XGBoost':
                y_pred_train, y_predprob_train, y_pred_test, y_predprob, train_time, test_time = \
                    train_XGB_model(clf, X_train, y_train, X_test, name)
            elif name == 'LinearSVC':
                y_pred_train, y_predprob_train, y_pred_test, y_predprob, train_time, test_time = \
                    train_SVM_model(clf, X_train_scaled, y_train, X_test_scaled, name)
            elif name == 'LogisticRegression':
                y_pred_train, y_predprob_train, y_pred_test, y_predprob, train_time, test_time = \
                    train_RF_LR_model(clf, X_train_scaled, y_train, X_test_scaled, name)
            else:
                y_pred_train, y_predprob_train, y_pred_test, y_predprob, train_time, test_time = \
                    train_RF_LR_model(clf, X_train, y_train, X_test, name)
            ...
            results = results.append(
                ...
                ignore_index=True)
    results.to_csv('Results/results_test.csv', sep=',', encoding='utf-8', index=False)
...
def start_10_fold_validation(df_):
    ...
    for index in range(len(df_split) - 1):
        ...
        for name, value in classifiers.items():
            ...
            reversefactor = dict(zip(range(3), definitions))
            y_test_1 = np.vectorize(reversefactor.get)(y_test)
            y_pred_test_1 = np.vectorize(reversefactor.get)(y_pred_test)

            print(pd.crosstab(y_test_1, y_pred_test_1, rownames=['Actual PRs'], colnames=['Predicted PRs']))

            print(metrics.classification_report(y_test_1, y_pred_test_1, digits=3))

            precision_avg, recall_avg, fscore_avg = extract_metric_from_report(
                metrics.classification_report(y_test_1, y_pred_test_1, digits=3))

            results = results.append(
                {'Model': name,
                 'P_respond': precision[1], 'P_accept': precision[0], 'P_reject': precision[2],
                 'R_respond': recall[1],
                 'R_accept': recall[0], 'R_reject': recall[2], 'f1_respond': fscore[1], 'f1_accept': fscore[0],
                 'f1_reject': fscore[2],
                 'Avg_Pre': precision_avg, 'Avg_Recall': recall_avg, 'Avg_f1_Score': fscore_avg,
    ...
    results.to_csv('Results/results_10F_test.csv', sep=',', encoding='utf-8', index=False)
    results_avg = calcuate_average_of_10_folds_1(results)
    results_avg.to_csv('Results/results_10F_avg_test.csv', sep=',', encoding='utf-8', index=False)
...
def start_each_project_model(df):
    ...
    for project in project_list:
        ...
        X_test = df_project.loc[(df_project['PR_Date_Created_At'] >= start_date) &
                                (df_project['PR_Date_Created_At'] <= end_date)]
        y_test = X_test[target]
...
def start_each_project_model_10_folds(df):
    results = pd.DataFrame(columns=['Model', 'Project', 'Fold', 'P_respond', 'P_accept', 'P_reject', 'R_respond', 'R_accept',
                                    'R_reject', 'f1_respond', 'f1_accept', 'f1_reject', 'Avg_Pre', 'Avg_Recall',
                                    'Avg_f1_Score', 'Test_Accuracy', 'Train_Accuracy', 'Training_time', 'Testing_time'])

    for project in project_list:
        df_project = df.loc[df.Project_Name == project]
        print('Project {} is under processing'.format(project))

        df_project = df_project.sort_values(by=['PR_Date_Closed_At', 'PR_Time_Closed_At'], ascending=True)
        df_split = np.array_split(df_project, 11)
        ...
        for index in range(len(df_split) - 1):
            ...
            for name, value in classifiers.items():
                ...
                results = results.append(
                    {'Model': name, 'Project': project, 'Fold': index,
                     'P_respond': precision[1], 'P_accept': precision[0], 'P_reject': precision[2],
                     'R_respond': recall[1],
                     'R_accept': recall[0], 'R_reject': recall[2], 'f1_respond': fscore[1], 'f1_accept': fscore[0],
                     'f1_reject': fscore[2],
    ...
    results.to_csv('Results/each_project_10F_test.csv', sep=',', encoding='utf-8', index=False)
    results_avg = calcuate_average_of_10_folds_for_each_project(results)
    results_avg.to_csv('Results/each_project_10F_avg_test.csv', sep=',', encoding='utf-8', index=False)
...
def calcuate_average_of_10_folds_1(df):
    avg_result = pd.DataFrame(columns=['Model', 'P_respond', 'P_accept', 'P_reject', 'R_respond', 'R_accept', 'R_reject',
                                    'f1_respond', 'f1_accept', 'f1_reject', 'Avg_Pre', 'Avg_Recall', 'Avg_f1_Score',
                                    'Test_Accuracy', 'Train_Accuracy', 'Training_time', 'Testing_time'])
    classifiers = get_classifiers()
    for name, value in classifiers.items():
        ...
        avg_result = avg_result.append(
            {'Model': name,
             'P_respond': model_result['P_respond'].mean(),
             'P_accept': model_result['P_accept'].mean(),
             'P_reject': model_result['P_reject'].mean(),
             'R_respond': model_result['R_respond'].mean(),
             'R_accept': model_result['R_accept'].mean(),
             'R_reject': model_result['R_reject'].mean(),
             'f1_respond': model_result['f1_respond'].mean(),
             'f1_accept': model_result['f1_accept'].mean(),
             'f1_reject': model_result['f1_reject'].mean(),
             'Avg_Pre': model_result['Avg_Pre'].mean(),
             'Avg_Recall': model_result['Avg_Recall'].mean(),
...
def calcuate_average_of_10_folds_for_each_project(df):
    avg_result = pd.DataFrame(columns=['Model', 'Project', 'P_respond', 'P_accept', 'P_reject', 'R_respond',
                                       'R_accept', 'R_reject', 'f1_respond', 'f1_accept', 'f1_reject', 'Avg_Pre',
                                       'Avg_Recall', 'Avg_f1_Score', 'Test_Accuracy', 'Train_Accuracy', 'Training_time',
                                       'Testing_time'])
    classifiers = get_classifiers()
    for project in project_list:
        df_project = df.loc[df.Project_Name == project]
