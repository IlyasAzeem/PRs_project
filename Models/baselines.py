...
def get_classifiers():
    return {
            'Randomforest': RandomForestClassifier(n_jobs=4, bootstrap=False, class_weight='balanced',
                                                   max_depth=15, min_samples_leaf=1, n_estimators=400),
            'LinearSVC': LinearSVC(max_iter=2500),
            'LogisticRegression': LogisticRegression(solver='lbfgs', n_jobs=4, multi_class='ovr', max_iter=1200),
...
df = df[['src_churn', 'Commits_PR', 'Files_Changed', 'num_comments', 'Participants_Count', 'conflict', 'Team_Size',
         'Project_Size', 'File_Touched_Average', 'Commits_Average', 'Prev_PRs', 'User_Accept_Rate', 'PR_Time_Created_At',
...
def train_XGB_model(clf, x_train, y_train, x_test):
    ...
    # plt.savefig('plots/XGB_f_gain_1.png', bbox_inches='tight')
...
def train_RF_LR_model(clf, x_train, y_train, x_test):
    clf.fit(x_train, y_train)
    with open('Saved_Models/3_labels/baseline.pickle.dat', 'wb') as f:
        pickle.dump(clf, f)
    # train
    y_pred_train = clf.predict(x_train)
    y_predprob_train = clf.predict_proba(x_train)[:, 1]
...
def start_train_models():
    ...
    for name, value in classifiers.items():
            ...
            if name == 'XGBoost':
                ...
                y_pred_train, y_predprob_train, y_pred, y_predprob = train_SVM_model(clf, X_train_scaled, y_train, X_test_scaled)
            elif name == 'LogisticRegression':
                y_pred_train, y_predprob_train, y_pred, y_predprob = train_RF_LR_model(clf, X_train_scaled, y_train, X_test_scaled)
            else:
            ...
            results.to_csv('Results/baseline_1.csv', sep=',', encoding='utf-8', index=False)
...
def start_each_project_model(df):
    ...
    results.to_csv('Results/results_projects_5.csv', sep=',', encoding='utf-8', index=False)
